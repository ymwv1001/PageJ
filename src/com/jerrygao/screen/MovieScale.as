package com.jerrygao.screen
{
	...
	public class MovieScale extends MovieClip
	{
		public function MovieScale()
		{
			super();
		}
		...
		*/
		...
		public var fFunc:Function;
		public function playFlv(url:String,volume:Number,replay:Boolean,finishFunc:Function = null):void
		{
			fFunc = finishFunc;
		...
		{ 
			...
			/*if(stage!=null)
			{
				onResized(stage.stageWidth,stage.stageHeight);
			}else
			{
			...
			{  
				...
				case "NetStream.Play.Stop":  
					...
					if(isReplay)
					{
						playFlv(Vurl,myVolume,isReplay,fFunc);
					}
					else
					{
						if(fFunc!=null)fFunc.call();
					}
					break;  
			}  
		}
		...
		public function onResized(w:Number,h:Number):void
		{
			...
			if(video&&this.contains(video))
			{
				...
				video.width = Global.stageW;
				video.height = Global.stageH;
				video.x = Global.displayX;
				video.y = Global.displayY;
				//video.alpha = 0.4;
				//trace("aa",w,h);
				//trace("bb",video.width,video.height);
				trace("onResized22222",video.width,video.height,video.videoWidth,video.videoHeight);
			}
		}
		/*
